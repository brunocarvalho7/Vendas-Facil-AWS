...
public class VendasMensal extends AsyncTask<Void, Void, Reports> {
    ...
    public VendasMensal(Context mContext){
    ...
    protected Reports doInBackground(Void... voids) {
        String dataAtual = new SimpleDateFormat("yyyy-MM").format(new Date());
        ...
        mDatabase.orderByChild("data").startAt(dataAtual).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                Reports report = new Reports(new SimpleDateFormat("M").format(new Date()));
